...
export function extractOffer(post) {
  const raw = post.title.replace(/^\[.*?\]\s*/, '').trim();
  // QDIS의 최근 제목 형식: "합격을 축하합니다_성균관대학교_2027학년도 전기"
  const t = raw.replace(/^합격을\s*축하합니다[\s_·:–-]*/i, '').replace(/_/g, ' ').trim();
  const intake = t.match(/(20\d{2})학년도\s*(전기|후기|수시|정시)?/);
  const academicYear = intake ? `${Number(intake[1]) - 1}-${intake[1]}` : null;
  const level = /석사|박사|대학원|master|ph\.?d/i.test(t) ? 'graduate' : 'undergraduate';
  const withoutIntake = intake ? t.replace(intake[0], '').trim() : t;
  const detailProgram = withoutIntake.match(/(의과대학|[가-힣]+학과|[가-힣]+학부)/)?.[0] ?? null;
  const program = [detailProgram, intake?.[0]].filter(Boolean).join(' · ') || null;
  const university = withoutIntake
    .replace(detailProgram ?? '', '')
    .replace(/\s*합격(?:을\s*축하합니다)?\s*$/i, '')
    .replace(/[-–·(\s]+$/, '')
    .trim();
  const country = UNI_COUNTRY.find(([r]) => r.test(university || t))?.[1] ?? null;
  return {academic_year: academicYear, country, university, program, level, result_type: level === 'graduate' ? 'alumni_graduate_offer' : 'individual_offer', offer_count: 1, source_url: post.url, source_post_id: post.no, source_date: post.date, verified: false};
}
